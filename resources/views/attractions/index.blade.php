...
<body class="min-h-screen bg-gray-50">
    ...
    <header class="fixed top-4 right-4 z-50">
        <nav class="flex items-center gap-3 rounded-lg bg-white/80 backdrop-blur-sm border border-gray-200 px-3 py-2 shadow-sm">
            @if($username)
                <span class="hidden sm:inline text-sm text-gray-500">Halo, <span class="font-medium text-gray-900">{{ $username }}</span></span>
            @endif
    ...
    <section class="relative overflow-hidden bg-gradient-to-br from-emerald-600 via-teal-600 to-cyan-600 py-20 px-6">
        ...
        <div class="relative max-w-5xl mx-auto text-center">
            ...
            <form method="GET" action="{{ url('/') }}" class="flex flex-col sm:flex-row gap-4 max-w-2xl mx-auto">
                <div class="relative flex-1">
                    ...
                    <input type="text" name="search" value="{{ $searchQuery }}" placeholder="Cari destinasi wisata..." class="w-full pl-12 pr-4 h-14 text-lg bg-white rounded-lg shadow-lg border-0 focus:ring-2 focus:ring-white/50 outline-none">
    ...
    <section class="container mx-auto px-4 py-6">
        <div class="flex flex-wrap gap-2">
            @foreach($categories as $cat)
                <a href="{{ url('/') }}?category={{ urlencode($cat) }}&search={{ urlencode($searchQuery) }}" class="px-4 py-2 rounded-full text-sm font-medium transition {{ $selectedCategory === $cat ? 'bg-emerald-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100 border border-gray-200' }}">
                    {{ $cat }}
                </a>
            @endforeach
        </div>
        ...
        <div class="grid lg:grid-cols-2 gap-8">
                ...
                <div id="attractionList" class="absolute top-20 left-4 bg-white/95 backdrop-blur-sm rounded-lg shadow-lg max-w-xs max-h-72 overflow-y-auto z-10">
                    ...
                    <div class="p-2 space-y-1">
                        @foreach($attractions as $a)
                            <button onclick="selectAttraction({{ json_encode($a) }})" class="w-full text-left p-2 rounded-md hover:bg-gray-100 transition attraction-item" data-id="{{ $a['id'] }}">
                                <div class="flex items-start gap-2">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <p class="text-xs text-gray-500 truncate">{{ $a['category'] }}</p>
                                        <div class="flex items-center gap-1 mt-0.5">
                                            ...
                                            <span class="text-xs">{{ $a['rating'] }}</span>
                                        </div>
                                    </div>
                                </div>
                            </button>
            ...
            <div class="space-y-6">
                <div class="flex items-center justify-between">
                    <h2 class="text-2xl font-bold text-gray-900">{{ count($attractions) }} Destinasi</h2>
                ...
                @forelse($attractions as $a)
                    <article class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition cursor-pointer" onclick="selectAttraction({{ json_encode($a) }})">
                        <div class="relative">
                            <img src="{{ $a['image'] }}" alt="{{ $a['name'] }}" class="w-full h-48 object-cover" onerror="this.src='https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=400&q=60'">
                            <div class="absolute top-3 right-3 flex gap-2">
                                <button onclick="event.stopPropagation(); openEditModal({{ json_encode($a) }})" class="p-2 bg-white/90 hover:bg-white rounded-full shadow transition" title="Edit">
                                ...
                                <form method="POST" action="{{ url('/attractions/' . $a['id']) }}" onclick="event.stopPropagation();" onsubmit="return confirm('Yakin ingin menghapus?')">
                                    @csrf
                                    @method('DELETE')
                        ...
                        <div class="p-4">
                            <div class="flex items-start justify-between mb-2">
                                <div>
                                    <h3 class="font-semibold text-lg text-gray-900">{{ $a['name'] }}</h3>
                                    <span class="inline-block px-2 py-0.5 bg-emerald-100 text-emerald-700 text-xs rounded-full mt-1">{{ $a['category'] }}</span>
                                </div>
                                <div class="flex items-center gap-1 text-sm">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-yellow-400 fill-yellow-400" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
                                    <span class="font-medium">{{ $a['rating'] }}</span>
                                </div>
                            </div>
                            <p class="text-gray-600 text-sm mb-2 line-clamp-2">{{ $a['description'] }}</p>
    ...
    <div id="formModal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <form id="attractionForm" method="POST" action="{{ url('/attractions') }}" class="p-4 space-y-4">
                @csrf
    ...
    <script>
        ...
        function openAddModal() {
            ...
            document.getElementById('attractionForm').action = '{{ url("/attractions") }}';
